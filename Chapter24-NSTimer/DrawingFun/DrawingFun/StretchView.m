...
{
    if (!_draggingTimer)
        _draggingTimer = [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(_updateScrollPosition:) userInfo:nil repeats:YES];

    [self _calculateInitialPointsUsingPoint:[self convertPoint:event.locationInWindow fromView:nil]];
    self.needsDisplay = YES;
}

- (void)_updateScrollPosition:(NSTimer *)timer
{
